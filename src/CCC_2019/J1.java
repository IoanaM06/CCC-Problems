...
2-point field goal, or a 1-point free throw. You know the number of each of these types of scoring
for the two teams: the Apples and the Bananas. Your job is to determine which team won, or if the game
ended in a tie.
...
public class J1 {
    int applesPoints, bananasPoints;
    ...
    public J1() throws FileNotFoundException {
        ...
        var file = new File("Data/CCC_2019/J1");
        var scanner = new Scanner(file);

        for (i = 0; i < 3; i++) apples[i] = scanner.nextInt();
        for (i = 0; i < 3; i++) bananas[i] = scanner.nextInt();
    }

    public String morePoints() {
        applesPoints = 3 * apples[0] + 2 * apples[1] + apples[2];
        bananasPoints = 3 * bananas[0] + 2 * bananas[1] + bananas[2];

        if (applesPoints == bananasPoints) return "T";
        if (applesPoints > bananasPoints) return "A";
        return "B";
    }
}
